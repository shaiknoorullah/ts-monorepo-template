...
export const lintCommand = defineCommand({
  args: {
    fix: { description: 'Apply auto-fixes (eslint --fix + prettier --write).', type: 'boolean' },
  },
  meta: { description: 'ESLint + Prettier --check + markdownlint + cspell.', name: 'lint' },
  async run({ args }) {
    if (args.fix) {
      ...
      emit({ message: 'Lint fixes applied.', status: 'ok' })
      return
    }
    ...
    emit({ message: 'Lint passed.', status: 'ok' })
  },
})
